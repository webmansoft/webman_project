...
class CodeHelper
{
     ...
     * @return string
     */
    public static function randomNicknameByUid(int $id): string
    {
        $idString = strval($id);
        $nickname = '';
        $arr = ['富', '强', '民', '主', '文', '明', '和', '谐', '自'];
        for ($i = 0; $i < strlen($idString); $i++) {
            $randStr = explode('.', '由.平.等.公.正.法.治.爱.国.敬.业.诚.信.友.善')[rand(0, 14)];
            $nickname .= $arr[$idString[$i]] . $randStr;
        }
    ...
    // 生成随机用户名
    // 测试，循环创建25万个随机账号，0碰撞，足够应付未来数十亿级PV
    public static function randomUserNameByTime(): string
    {
     ...
     * @return string
     */
    public static function supplementId(string $id): string
    {
        $len = strlen($id);
        $buf = '000000';
     ...
     * @return string
     */
    public static function createOrderId(string $letter = '', int $length = 3): string
    {
        $gradual = 0;
        ...
        if (empty($letter)) {
            $letter = self::getOrderLetter($length);
        }
     ...
     * @return string
     */
    public static function getOrderLetter(int $length = 2): string
    {
     ...
     * @return string
     */
    public static function createOrderShortId(string $letter = '', int $length = 5): string
    {
        if (empty($letter)) {
            $letter = self::getOrderLetter($length);
        }

        return $letter . date('Ymd') . substr(implode('', array_map('ord', str_split(substr(uniqid(), 7, 13)))), 0, 12);
    }
    ...
    {
        $str = strval(microtime(true));
        $time = substr($str, strpos($str, '.') + 1);
        return 'JS' . strtoupper(base_convert(strval(time() - 1420070400), 10, 36)) . $time;
    }
}